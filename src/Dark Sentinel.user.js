...
(function () {
	...
	function carregarDadosServidor() {
		...
		cacheCidadesPorIlha = {};
		const t = Date.now();
		$.ajax({ method: 'get', url: '/data/towns.txt?_=' + t }).done(function (m) {
			try {
				$.each(m.split(/\r\n|\n/), function (C, J) {
					if (J.trim()) {
						const cols = J.split(/,/);
		...
		$.ajax({ method: 'get', url: '/data/players.txt?_=' + t }).done(function (m) {
			try {
				$.each(m.split(/\r\n|\n/), function (C, J) {
					if (J.trim()) {
						const cols = J.split(/,/);
		...
		$.ajax({ method: 'get', url: '/data/alliances.txt?_=' + t }).done(function (m) {
			try {
				$.each(m.split(/\r\n|\n/), function (C, J) {
					if (J.trim()) {
						const cols = J.split(/,/);
	...
	function sendSentinelaOtherIsland(cidadeAlvoId) {
		const cfg = carregarConfig();
		const currentTown = uw.ITowns.getCurrentTown();
		if (!currentTown) { uw.HumanMessage.error('Nenhuma cidade atual'); return; }
		const cid = String(currentTown.id);
		const unitTerra = obterUnidadeParaCidade(cid, cfg);
		if (unitTerra) { enviarSentinela(unitTerra, cidadeAlvoId, cid); return; }
		const unitNaval = obterUnidadeNaval(cid, cfg);
		if (unitNaval) { enviarSentinela(unitNaval, cidadeAlvoId, cid); return; }
	...
	uw.$.Observer(uw.GameEvents.map.town.click).subscribe((e, data) => {
		...
		<div style="color: #aaa; font-size: 10px; text-align: center; padding: 2px 4px; position: relative; z-index: 1;">` + (cidadeEstaNaIlha(data.x, data.y) ? '' : 'De: ' + (function () { try { var ct = uw.ITowns.getCurrentTown(); return typeof ct.get === 'function' ? ct.get('name') : ''; } catch (e) {} return ''; })()) + `</div>
		</div>`;
		menu.append(div);
	...
	const SHIELD_URL = 'https://gpit.innogamescdn.com/images/game/autogenerated/map/town_overlay/city_shield_cd2b0df.png';
	const shieldCache = {};
	let shieldImageLoaded = null;
	...
	function gerarEscudoColorido(img, hue) {
		...
		for (let i = 0; i < data.length; i += 4) {
			...
			if (hue === -1) {
				hsl[1] = 0;
				hsl[2] = Math.min(100, hsl[2] + 40);
			} else {
				hsl[0] = (hsl[0] + hue) % 360;
				if (hsl[0] < 0) hsl[0] += 360;
				hsl[1] = Math.min(100, hsl[1] * 1.8);
			}
			const rgb = hslParaRgb(hsl[0], hsl[1], hsl[2]);
	...
	function obterUrlEscudo(hue) {
		if (shieldCache[hue]) return Promise.resolve(shieldCache[hue]);
		return carregarImagemEscudo().then(function (img) {
			if (!img) return SHIELD_URL;
			const url = gerarEscudoColorido(img, hue);
			shieldCache[hue] = url;
			return url;
		});
	}

	function preGerarEscudos() {
		const hues = [210, 90, 330, 240, 30, 300, -1];
		carregarImagemEscudo().then(function (img) {
			if (!img) return;
			hues.forEach(function (hue) {
				if (!shieldCache[hue]) shieldCache[hue] = gerarEscudoColorido(img, hue);
			});
			dsDebug('Escudos pre-gerados');
		});
	}
	...
	function adicionarEscudoVerde(cidadeId) {
		...
		const opacity = (typeof cfg.opacidadeEscudo === 'number' ? cfg.opacidadeEscudo : 100) / 100;
		...
		shield.style.opacity = opacity;
		map.appendChild(shield);
		if (hue === 0) {
			shield.style.background = `url(${SHIELD_URL}) no-repeat 0 0`;
		} else {
			obterUrlEscudo(hue).then(function (url) {
				if (shield.parentElement) shield.style.background = `url(${url}) no-repeat 0 0`;
			});
		}
		return true;
	}
	...
	function atualizarCorEscudos() {
		...
		const hue = typeof cfg.corEscudo === 'number' ? cfg.corEscudo : 210;
		const opacity = (typeof cfg.opacidadeEscudo === 'number' ? cfg.opacidadeEscudo : 100) / 100;
		const shields = document.querySelectorAll('[id^="s4_"]');
		if (hue === 0 || shieldCache[hue]) {
			const url = hue === 0 ? SHIELD_URL : shieldCache[hue];
			shields.forEach(function (el) {
				el.style.filter = '';
				el.style.opacity = opacity;
				el.style.background = `url(${url}) no-repeat 0 0`;
			});
			return;
		}
		obterUrlEscudo(hue).then(function (url) {
	...
	/* Retorna o nome da cidade pelo ID */
	function cidadeIdParaNome(id) {
		for (let i = 0; i < lista_cidades.length; i++) {
			if (String(lista_cidades[i][0]) === String(id)) return lista_cidades[i][2] || 'Cidade ' + id;
		}
		return null;
	}
	...
	function carregarConfig() {
		try {
			...
			return { terra: terra, naval: naval, ordemTerra: PADRAO_ORDEM_TERRA.slice(), ordemNaval: PADRAO_ORDEM_NAVAL.slice(), listaNegraAtiva: true, overlayAtivo: true, enviarAliados: true, enviarSemAlianca: true, enviarFantasmas: true, enviarInimigos: false, ilhasIgnoradas: [], modoEnvio: 'filtro', aliancasSelecionadas: [], corEscudo: 210, opacidadeEscudo: 100, envioOutrasIlhas: 0 };
		}
	}
	...
	function abrirConfig() {
		...
		const secOutrasIlhas = document.createElement('div');
		secOutrasIlhas.style.cssText = 'display:flex;align-items:flex-start;justify-content:space-between;margin:8px 0;padding:6px;border-radius:4px;';
		const infoOutras = document.createElement('div');
		infoOutras.style.cssText = 'flex:1;margin-right:10px;text-align:left;';
		const titOutras = document.createElement('div');
		titOutras.style.cssText = 'font-size:12px;font-weight:bold;color:#fc6;margin-bottom:2px;';
		titOutras.textContent = 'Envio p/ Outras Ilhas';
		const descOutras = document.createElement('div');
		descOutras.style.cssText = 'font-size:10px;color:#aaa;line-height:1.3;';
		descOutras.textContent = 'Permite enviar sentinelas para cidades de outras ilhas.';
		infoOutras.appendChild(titOutras);
		infoOutras.appendChild(descOutras);
		const selectOutras = document.createElement('select');
		selectOutras.style.cssText = 'padding:4px 6px;background:#1a1a1a;border:1px solid #8b6914;border-radius:3px;color:#fc6;font-size:12px;cursor:pointer;flex-shrink:0;margin-top:2px;';
		const opcoesOutras = [
			{ label: 'Não enviar', valor: 0 },
			{ label: 'Menu de Contexto', valor: 1 },
			{ label: 'Contexto + Ilha', valor: 2 }
		];
		const atualOutras = typeof cfg.envioOutrasIlhas === 'number' ? cfg.envioOutrasIlhas : 0;
		opcoesOutras.forEach(function (o) {
			const opt = document.createElement('option');
			opt.value = o.valor;
			opt.textContent = o.label;
			if (o.valor === atualOutras) opt.selected = true;
			selectOutras.appendChild(opt);
		});
		selectOutras.onchange = function () {
			cfg.envioOutrasIlhas = parseInt(selectOutras.value);
			salvarConfig(cfg);
		};
		secOutrasIlhas.appendChild(infoOutras);
		secOutrasIlhas.appendChild(selectOutras);
		secSeg.appendChild(secOutrasIlhas);
		...
		const controlesCor = document.createElement('div');
		controlesCor.style.cssText = 'display:flex;align-items:center;gap:8px;flex-shrink:0;margin-top:2px;';
		...
		controlesCor.appendChild(selectCor);
		const opcLabel = document.createElement('span');
		opcLabel.style.cssText = 'font-size:10px;color:#aaa;';
		opcLabel.textContent = 'Opac';
		const inputOpacidade = document.createElement('input');
		inputOpacidade.type = 'range';
		inputOpacidade.min = '10';
		inputOpacidade.max = '100';
		inputOpacidade.value = String(typeof cfg.opacidadeEscudo === 'number' ? cfg.opacidadeEscudo : 100);
		inputOpacidade.style.cssText = 'width:50px;cursor:pointer;accent-color:#fc6;';
		const valorOpacidade = document.createElement('span');
		valorOpacidade.style.cssText = 'font-size:10px;color:#fc6;min-width:24px;text-align:right;';
		valorOpacidade.textContent = inputOpacidade.value + '%';
		inputOpacidade.oninput = function () {
			valorOpacidade.textContent = inputOpacidade.value + '%';
			cfg.opacidadeEscudo = parseInt(inputOpacidade.value);
			salvarConfig(cfg);
			atualizarCorEscudos();
		};
		controlesCor.appendChild(opcLabel);
		controlesCor.appendChild(inputOpacidade);
		controlesCor.appendChild(valorOpacidade);
		linhaCor.appendChild(infoCor);
		linhaCor.appendChild(controlesCor);
		...
		btnRestaurar.onclick = function (e) {
			...
			btnSim.onclick = function () {
				...
				for (let i = 0; i < UNIDADES_TERRA.length; i++) terra[UNIDADES_TERRA[i].chave] = PADRAO_TERRA[UNIDADES_TERRA[i].chave];
				for (let i = 0; i < UNIDADES_NAVAL.length; i++) naval[UNIDADES_NAVAL[i].chave] = PADRAO_NAVAL[UNIDADES_NAVAL[i].chave];
				salvarConfig({ terra: terra, naval: naval, ordemTerra: PADRAO_ORDEM_TERRA.slice(), ordemNaval: PADRAO_ORDEM_NAVAL.slice(), listaNegraAtiva: true, overlayAtivo: true, enviarAliados: true, enviarSemAlianca: true, enviarFantasmas: true, enviarInimigos: false, ilhasIgnoradas: [], modoEnvio: 'filtro', aliancasSelecionadas: [], corEscudo: 210, opacidadeEscudo: 100, envioOutrasIlhas: 0 });
				confirmModal.remove();
				modal.remove();
	...
	function injetarBotoesIlha() {
		...
		for (let e in wnds) {
			if (wnds.hasOwnProperty(e)) {
				...
				function confirmarEnvioOutraIlha(nomeCidade, lista, cid, callback) {
			...
			box.style.cssText = 'background:#222;border:2px solid #8b6914;border-radius:6px;padding:16px;min-width:300px;text-align:center;';
			var msg = document.createElement('div');
			msg.style.cssText = 'color:#fc6;font-size:13px;margin-bottom:12px;line-height:1.5;';
			var alvos = lista.filter(function (id) { return !temSentinela(id) && !estaNaListaNegra(id); });
			msg.innerHTML = 'Enviar sentinelas para <b>' + alvos.length + '</b> cidades<br>usando tropas de<br><b>' + nomeCidade + '</b>';
			var btnSim = document.createElement('div');
			btnSim.className = 'button_new a1';
			btnSim.style.cssText = 'display:inline-block;margin:4px;';
			btnSim.innerHTML = '<div class="left"></div><div class="right"></div><div class="caption js-caption">Enviar<div class="effect js-effect"></div></div>';
			var btnNao = document.createElement('div');
			btnNao.className = 'button_new a3';
			btnNao.style.cssText = 'display:inline-block;margin:4px;';
			btnNao.innerHTML = '<div class="left"></div><div class="right"></div><div class="caption js-caption">Cancelar<div class="effect js-effect"></div></div>';
			btnNao.onclick = function () { modal.remove(); };
			btnSim.onclick = function () { modal.remove(); if (callback) callback(); };
			box.appendChild(msg);
		...
		$(`#gpwnd_${wndid}`).on('click', '.a1', function () {
					...
					if (cidades_jogador.length === 0) {
						if (cfg.envioOutrasIlhas !== 2) {
							uw.HumanMessage.error('Nenhuma cidade sua nesta ilha');
							return;
						}
						const currentTown = uw.ITowns.getCurrentTown();
						if (!currentTown) { uw.HumanMessage.error('Nenhuma cidade atual'); return; }
						...
						const qtde = lista.filter(function (id) { return !temSentinela(id) && !estaNaListaNegra(id); }).length;
						if (qtde === 0) { uw.HumanMessage.error('Nenhum alvo encontrado'); return; }
						var nomeCidade = cidadeIdParaNome(cid) || cid;
						confirmarEnvioOutraIlha(nomeCidade, lista, cid, function () {
							for (let i = 0; i < lista.length; i++) {
								if (temSentinela(lista[i])) continue;
								if (estaNaListaNegra(lista[i])) continue;
								setTimeout(function () {
									const id = lista[i];
									if (temSentinela(id)) return;
									if (temSuporteACaminho(id)) return;
									if (estaNaListaNegra(id)) return;
									const unitTerra = obterUnidadeParaCidade(cid, cfg);
									if (unitTerra) { enviarSentinela(unitTerra, id, cid); return; }
									const unitNaval = obterUnidadeNaval(cid, cfg);
									if (unitNaval) { enviarSentinela(unitNaval, id, cid); return; }
								}, i * 500);
							}
						});
					}
